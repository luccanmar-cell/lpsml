...
- Keeps `Pol6TTaCod` and `CoberturaLabel` as readable reporting columns, while
  using a single one-hot encoding of their joint tariff-coverage pair as model
  input.

Always train from the clean output generated by the current `build_dataset.py`.
...
Hyperparameter cross-validation uses the same pair labels with
`StratifiedKFold`. Each candidate is evaluated from complete out-of-fold
predictions, and Optuna minimizes the largest final-Prima MAPE among the
tariff-coverage pairs. This is a minimax worst-group-risk objective: every pair
has to improve for the selection score to improve.
...
#### Uniform pair-performance training design

The final training design is:

- One `TariffCoverage_<tariff>__<coverage>` training indicator for each retained
  pair.
- Readable `Pol6TTaCod` and `CoberturaLabel` values in reports, with pair
  indicators excluded from scored and counterfactual outputs.
- Pair-stratified holdout and `StratifiedKFold` hyperparameter folds.
- Complete out-of-fold final-Prima MAPE calculated separately for every pair.
- A minimax selection score of `max(pair MAPE)`, minimizing empirical
  worst-pair risk.
- Independent hyperparameter searches for Random Forest, Extra Trees, and
  PyTorch. The best configuration of each model type is evaluated under the
  same objective, and the model with the lowest cross-validated worst-pair MAPE
  is selected overall.

The selection score is computed after assembling one out-of-fold prediction for
every training row; it is not an average of per-fold maxima. MAPE tuning requires
strictly positive final premiums and stops with an error if that condition is
violated.

Run summaries record `cv_worst_pair_mape_percent` for each model and
`overall_best_cv_worst_pair_mape_percent` for the selected model.
...
- Conserva `Pol6TTaCod` y `CoberturaLabel` como columnas legibles para reportes,
  y usa como entrada del modelo una única codificación one-hot del par
  tarifa-cobertura.

Entrenar siempre con el archivo limpio generado por la versión actual de
`build_dataset.py`.
...
La validación cruzada de hiperparámetros usa las mismas etiquetas de pares con
`StratifiedKFold`. Cada candidato se evalúa a partir de predicciones completas
fuera de muestra y Optuna minimiza el mayor MAPE de prima final entre los pares
tarifa-cobertura. Es un objetivo minimax de riesgo del peor grupo: todos los
pares deben mejorar para reducir el puntaje de selección.
...
#### Diseño de entrenamiento para rendimiento uniforme por par

El diseño final de entrenamiento es:

- Un indicador `TariffCoverage_<tarifa>__<cobertura>` de entrenamiento por cada
  par retenido.
- Valores legibles de `Pol6TTaCod` y `CoberturaLabel` en los reportes, sin
  indicadores de pares en las salidas puntuadas ni contrafactuales.
- Partición de evaluación y pliegues `StratifiedKFold` estratificados por par.
- MAPE de prima final fuera de muestra calculado por separado para cada par.
- Puntaje minimax `max(MAPE por par)`, que minimiza el riesgo empírico del peor
  par.
- Búsquedas de hiperparámetros independientes para Random Forest, Extra Trees y
  PyTorch. La mejor configuración de cada tipo se evalúa con el mismo objetivo,
  y se selecciona el modelo con el menor MAPE del peor par en validación
  cruzada.

El puntaje se calcula después de reunir exactamente una predicción fuera de
muestra para cada fila de entrenamiento; no es el promedio de los máximos de
cada pliegue. La optimización por MAPE requiere primas finales estrictamente
positivas y se detiene con un error si no se cumple esa condición.

Los resúmenes guardan `cv_worst_pair_mape_percent` para cada modelo y
`overall_best_cv_worst_pair_mape_percent` para el modelo seleccionado.
